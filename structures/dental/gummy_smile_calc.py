...
def find_interesting_points(non_black_pixels_mask, middle_point, break_parameter=9):
    total_count = 0
    points_of_interest = []
    while total_count != 3:
        before = 0
...
def draw_lines_and_save(img_path, output_path):
    ...
    points_of_interest_right = find_interesting_points(
        non_black_pixels_mask[:, ::-1], width - middle_point
    )
