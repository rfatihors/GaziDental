...
def create_mask(json_path: str, image_id: int, output_dir: str) -> str:
    ...
    target_image_data = next((img for img in coco_data["images"] if img["id"] == image_id), None)
    if not target_image_data:
        raise ValueError(f"Belirtilen image_id'ye sahip görsel bulunamadı: {image_id}")
    ...
    mask_path = os.path.join(output_dir, f"{target_image_data['file_name']}_mask.bmp")
    mask.save(mask_path)
    return mask_path
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--image-id", type=int, default=1, help="Hedef görselin ID değeri")
    parser.add_argument("--output-dir", default=default_masks, help="Maskelerin kaydedileceği klasör")
