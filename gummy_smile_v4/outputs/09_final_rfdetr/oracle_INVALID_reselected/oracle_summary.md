...
## Protocol
selection on dev MAE (n = 87); best dev MAE = 0.515 mm; 5 combination(s) within 0.02 mm of it (B_p05, B_min, B_p05_lipanchored, B_min_lipanchored, C_p25); the simplest of those is chosen (A > B > C, gingiva thickness > lip-anchored, p25 > median > p10 > p05 > min > max).
...
## Selected method: **B_p05**, global scale **13.55 px/mm**
Holdout (n = 58): MAE 0.543 mm, RMSE 0.742 mm, r 0.866, ICC(2,1) 0.857 [0.768, 0.913], bias +0.181 mm [-0.010, +0.372], LoA -1.24 to 1.60 mm, proportional bias slope +0.104 (p = 0.150); threshold-label agreement 75.9 % (linear-weighted κ 0.731) — a *measurement* check against Table 1, not a clinical validation.
Dev (n = 87): MAE 0.523 mm, r 0.897, ICC(2,1) 0.888.

**Fallback transparency.** Regioning B could not be established on 2 of 145 images (1 %, `festoon_detection_failed`); there the measurement silently uses the equal-split regions (A) and the value is identical to `A_p05`. On the dev images where B succeeded (n = 85), dev MAE is 0.526 mm for `B_p05` vs 0.580 mm for `A_p05` — the advantage of B comes from these images, not from the fallback ones.

Fallback distribution (2 images): zenith candidates found left+right of the midline (3+3 needed): 2+1: 1, 2+2: 1. Total minima on fallback images: median 3 vs 6 on successful ones. Gingiva band width (fraction of image width, a proxy for premolar visibility): fallback 0.254 vs success 0.343; reference mm: fallback 0.90 vs success 2.87. Fallback images are not wider, so premolar visibility is not the main cause; the typical failure is one side of the midline having fewer than three detectable minima (a shallow festoon on that side).

**Sensitivity analysis — `A_p05` (no fallback, equal-split regions) side by side:** holdout MAE 0.539 vs 0.543 mm, RMSE 0.720 vs 0.742, r 0.862 vs 0.866, ICC(2,1) 0.857 vs 0.857, bias +0.148 vs +0.181 mm, scale 13.74 vs 13.55 px/mm.

**Stage 6 note:** on predicted masks the fallback rate of `B_p05` will be re-measured; if it exceeds 30 % the selection is re-evaluated against `A_p05`.

**Against the reference's own repeatability:** intra-observer SD is 0.17 mm per tooth site and 0.09 mm per image mean (`intra_observer.md`); pure observer noise would produce an expected absolute difference of ≈ 0.07 mm at image level. The holdout MAE of 0.54 mm therefore leaves ≈ 0.47 mm above the observer-noise floor, attributable to the estimator, the single global scale (per-image calibration was not recorded) and region alignment.

Pre-analysis plausibility check (audit §6, A/p25, same-data scale, n = 148): r ≈ 0.83, MAE ≈ 0.63 mm, ≈ 17 px/mm — DIFFERENT — see OZET.
...
Sign of `ba_bias`: positive = GT-mask value above the clinical reference. `ba_prop_slope` is the slope of (GT − ref) on the mean of both; p < 0.05 indicates a proportional (mm-dependent) bias. For the selected method: bias grows with gingival display (slope +0.104, p = 0.150).
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| holdout: all | 58 | 0.543 | 0.742 | 0.866 | 0.000 | 0.857 | 0.768 | 0.913 | 0.862 | 0.181 | -0.010 | 0.372 | 0.726 | -1.241 | 1.604 | 0.104 | 0.150 | 0.759 | 0.731 |
| holdout: without dash-zero rows | 45 | 0.552 | 0.697 | 0.871 | 0.000 | 0.861 | 0.762 | 0.921 | 0.862 | 0.125 | -0.083 | 0.333 | 0.693 | -1.234 | 1.484 | 0.154 | 0.059 | 0.756 | 0.750 |
| holdout: only 2698x1799 (+/-2 px) frames | 48 | 0.500 | 0.653 | 0.892 | 0.000 | 0.887 | 0.807 | 0.935 | 0.886 | 0.083 | -0.107 | 0.273 | 0.654 | -1.200 | 1.365 | 0.123 | 0.086 | 0.771 | 0.733 |
| holdout: frames outside 2698x1799 | 10 | 0.748 | 1.071 | 0.784 | 0.007 | 0.698 | 0.141 | 0.917 | 0.769 | 0.654 | 0.015 | 1.294 | 0.894 | -1.098 | 2.407 | -0.220 | 0.393 | 0.700 | 0.706 |
| holdout: without ambiguous 100-999 cells | 58 | 0.543 | 0.742 | 0.866 | 0.000 | 0.857 | 0.768 | 0.913 | 0.862 | 0.181 | -0.010 | 0.372 | 0.726 | -1.241 | 1.604 | 0.104 | 0.150 | 0.759 | 0.731 |
| dev + holdout (scale still from dev) | 145 | 0.531 | 0.735 | 0.882 | 0.000 | 0.876 | 0.828 | 0.911 | 0.882 | 0.176 | 0.058 | 0.293 | 0.716 | -1.228 | 1.579 | -0.036 | 0.385 | 0.766 | 0.734 |
...
Lip-anchored estimators (B_p05_lipanchored): dev MAE 0.531 mm vs 0.523 mm for gingiva thickness.

## Tooth level (secondary; region i left-to-right vs reference tooth i; A/B/C alignment is approximate)
| tooth_index | n | mae | bias | r |
|---|---|---|---|---|
| 1 | 145 | 1.265 | -0.839 | 0.631 |
| 2 | 145 | 1.308 | 0.603 | 0.808 |
| 3 | 145 | 1.064 | 0.921 | 0.855 |
| 4 | 145 | 0.995 | 0.716 | 0.824 |
| 5 | 145 | 1.253 | 0.257 | 0.624 |
| 6 | 145 | 1.055 | -0.603 | 0.659 |
