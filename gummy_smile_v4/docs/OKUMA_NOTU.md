...
- **v3 COCO dışa aktarımında veri kalitesi sorunu (17 Eylül 2026'da bulundu):** `../gummy_smile_v3/data/coco_dataset/*/*/_annotations.coco.json` içindeki **5.256 annotation'ın hepsinde** `bbox` şu biçimde: `[int, int, str, str]` — x ve y sayı, genişlik ve yükseklik biçimlendirilmiş metin (`'1300.0000'`). `area` kimi yerde int kimi yerde float; poligon koordinatları da karışık. Kaynaktan geliyor, bizim ürettiğimiz bir hata değil. Bugüne kadar fark edilmemesinin nedeni: bizim hattımız `bbox` alanını **hiç okumuyor**, yalnızca `segmentation` poligonlarını rasterleştiriyor (`prepare_yolo_dataset.py`, `masks/extract.py`). İlk tüketici RF-DETR'in yükleyicisi oldu ve `torch.as_tensor(bbox, dtype=torch.float32)` adımında `TypeError: must be real number, not str` verdi. `build_rfdetr_dataset.py` artık her sayısal alanı açıkça dönüştürüyor, yazmadan önce doğruluyor ve yazdıktan sonra pycocotools ile yeniden açıyor. COCO'dan yeni bir tüketici eklenirse aynı kontrolden geçirilmeli.
- Post-hoc kalibrasyon (16 Eylül 2026), **maske düzeyi**: `measurement.bottom_edge_offset_px = -13` — tahmin maskesinde dişeti alt kenarı her sütunda 13 px yukarı çekilir, profil ve tüm tahminciler düzeltilmiş maskeden hesaplanır; 13 px'ten ince sütun 0 olur (negatif değil) ve sayılır. Aşama 3 dev listesinde maske düzeyi taramayla kestirildi, holdout ve test setinde doğrulandı (`06_prediction/offset_correction.md`: üç varyantın tablosu ve seçim gerekçesi; `offset_checks.md`). Değer düzeyi `offset_px` anahtarı kaldırıldı (yükleyici reddeder). Piksel cinsinden olduğu için görüntü bazlı ölçekle (Aşama 4) doğrudan çalışır. **Düzeltmesiz = birincil, düzeltilmiş = ikincil; her tabloda ikisi de, birincil önce.**
